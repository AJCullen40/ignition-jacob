...
const STATIC_CACHE_URL = "/data/commbook-cache.json"; // Served from public/data/
...
async function getCachedData(startDate?: string, endDate?: string): Promise<CommBookData | null> {
  try {
    ...
    if (!cacheData) {
      // Try static file bundled with deployment (via HTTP)
      try {
        // Use the canonical URL to avoid auth redirect issues
        const cacheUrl = "https://lawsb.ignitionsystems.io/data/commbook-cache.json";
        console.log(`[CB] Fetching static cache from ${cacheUrl}`);
        const resp = await fetch(cacheUrl, {
          cache: "no-store",
          signal: AbortSignal.timeout(5000),
          redirect: "follow",
        });
        console.log(`[CB] Static cache response: ${resp.status}`);
        if (resp.ok) {
          ...
          if (text.startsWith("{")) {
            ...
            if (cacheData) {
              globalThis.__commBookCache = { data: cacheData, ts: Date.now() };
              console.log(`[CB] Loaded from static deployment cache: ${cacheData.generatedAt}`);
            }
          }
        }
      } catch (e) {
        console.warn("[CB] Static cache fetch failed:", e);
      }
    }
...
const AREA_CODE_MAP: Record<string, string> = {
  ...
  "337":"LA","339":"MA","340":"VI","346":"TX","347":"NY","351":"MA","352":"FL","360":"WA",
  "361":"TX","385":"UT","386":"FL","401":"RI","402":"NE","404":"GA","405":"OK","406":"MT",
  "407":"FL","408":"CA","409":"TX","410":"MD","412":"PA","413":"MA","414":"WI","415":"CA",
  "417":"MO","419":"OH","423":"TN","424":"CA","425":"WA","430":"TX","432":"TX","434":"VA",
  "435":"UT","440":"OH","442":"CA","443":"MD","458":"OR","469":"TX","470":"GA","475":"CT",
  ...
  "540":"VA","541":"OR","551":"NJ","559":"CA","561":"FL","562":"CA","563":"IA","567":"OH",
  "570":"PA","571":"VA","573":"MO","574":"IN","575":"NM","580":"OK","585":"NY","586":"MI",
  "601":"MS","602":"AZ","603":"NH","605":"SD","606":"KY","607":"NY","608":"WI","609":"NJ",
  "610":"PA","612":"MN","614":"OH","615":"TN","616":"MI","617":"MA","618":"IL","619":"CA",
  "620":"KS","623":"AZ","626":"CA","628":"CA","629":"TN","630":"IL","631":"NY","636":"MO",
  "641":"IA","646":"NY","650":"CA","651":"MN","657":"CA","660":"MO","661":"CA","662":"MS",
  "667":"MD","669":"CA","678":"GA","681":"WV","682":"TX","689":"FL","701":"ND","702":"NV",
  "703":"VA","704":"NC","706":"GA","707":"CA","708":"IL","712":"IA","713":"TX","714":"CA",
  "715":"WI","716":"NY","717":"PA","718":"NY","719":"CO","720":"CO","724":"PA","725":"NV",
  "727":"FL","731":"TN","732":"NJ","734":"MI","737":"TX","740":"OH","743":"NC","747":"CA",
  ...
  "828":"NC","830":"TX","831":"CA","832":"TX","838":"NY","843":"SC","845":"NY","847":"IL",
  "848":"NJ","850":"FL","854":"SC","856":"NJ","857":"MA","858":"CA","859":"KY","860":"CT",
  "862":"NJ","863":"FL","864":"SC","865":"TN","870":"AR","872":"IL","878":"PA",
  "901":"TN","903":"TX","904":"FL","906":"MI","907":"AK","908":"NJ","909":"CA","910":"NC",
  "912":"GA","913":"KS","914":"NY","915":"TX","916":"CA","917":"NY","918":"OK","919":"NC",
  "920":"WI","925":"CA","928":"AZ","929":"NY","931":"TN","936":"TX","937":"OH","938":"AL",
  "940":"TX","941":"FL","947":"MI","949":"CA","951":"CA","952":"MN","954":"FL","956":"TX",
  "959":"CT","970":"CO","971":"OR","972":"TX","973":"NJ","978":"MA","979":"TX","980":"NC",
  "984":"NC","985":"LA",
};
const STATE_NAMES: Record<string, string> = {
  ...
  TN:"Tennessee",TX:"Texas",UT:"Utah",VT:"Vermont",VA:"Virginia",VI:"US Virgin Islands",
  WA:"Washington",WV:"West Virginia",WI:"Wisconsin",WY:"Wyoming",
};
const PLAT_COLORS: Record<string, string> = {
  Facebook: "#1877F2", Instagram: "#E4405F", YouTube: "#FF0000",
  TikTok: "#000000", Other: "#767676", Unknown: "#767676",
};
...
function extractVisa(text: string): string {
  ...
  for (const [t, kw] of Object.entries(VISA_TYPES)) { if (kw.some(k => l.includes(k))) return t; }
  return "Other";
}
...
function parseCSV(raw: string): Record<string, string>[] {
  ...
  const rows: string[][] = [];
  let cr: string[] = []; let cf = ''; let inQ = false; let i = 0;
  while (i < csv.length) {
    const ch = csv[i];
    if (inQ) {
      if (ch === '"') { if (csv[i+1] === '"') { cf += '"'; i += 2; } else { inQ = false; i++; } }
      else { cf += ch; i++; }
    } else if (ch === '"') { inQ = true; i++; }
    else if (ch === ',') { cr.push(cf); cf = ''; i++; }
    else if (ch === '\r' || ch === '\n') {
      cr.push(cf); cf = ''; rows.push(cr); cr = [];
      if (ch === '\r' && csv[i+1] === '\n') i += 2; else i++;
    } else { cf += ch; i++; }
  }
  if (cf || cr.length) { cr.push(cf); rows.push(cr); }
...
// ====== Caches ======
interface SFOppInfo { stage: string; amount: number; closeDate: string; caseType: string; name: string; }
interface ScoringWithSF {
  ghlId: string; sfId: string; name: string; phone: string; phoneOrigin: string;
  score: number; leadSource: string; dateCreated: string;
  opps: SFOppInfo[];
}

let scoringSFCache: { data: ScoringWithSF[]; ts: number } | null = null;
const SCORING_SF_TTL = 30 * 60 * 1000;
const resCache = new Map<string, { data: CommBookData; ts: number }>();
const RES_TTL = 5 * 60 * 1000;
let warmInProgress = false;

async function getScoringSFData(): Promise<ScoringWithSF[]> {
  if (scoringSFCache && (Date.now() - scoringSFCache.ts) < SCORING_SF_TTL) return scoringSFCache.data;
  if (warmInProgress) return scoringSFCache?.data || [];
  warmInProgress = true;

  try {
    const [scorAD, scorFH, scorJL] = await Promise.all([
      fetchSheet(SCORING_SHEET_ID, '0', 'A:D'),
      fetchSheet(SCORING_SHEET_ID, '0', 'F:H'),
      fetchSheet(SCORING_SHEET_ID, '0', 'J:L'),
    ]);

    const sfIdSet = new Set<string>();
    const scoringRows: { ghlId: string; name: string; score: number; sfId: string; phone: string; phoneOrigin: string; leadSource: string; dateCreated: string }[] = [];
    const len = Math.min(scorAD.length, scorFH.length, scorJL.length);
    
    for (let i = 0; i < len; i++) {
      const ghlId = (scorAD[i]["ID"] || "").trim();
      if (!ghlId) continue;
      const sfId = (scorJL[i]["SF ID"] || "").trim();
      if (sfId && sfId.startsWith("00")) sfIdSet.add(sfId);
      scoringRows.push({
        ghlId,
    ...
    const sfIds = Array.from(sfIdSet);
    console.log(`[CB] Scoring: ${scoringRows.length} rows, ${sfIds.length} SF IDs`);

    const B = 100;
    const lead2acct = new Map<string, string>();
    const accts = new Set<string>();

    const leadPromises: Promise<{ Id: string; ConvertedAccountId: string | null; IsConverted: boolean }[]>[] = [];
    for (let i = 0; i < sfIds.length; i += B) {
      const batch = sfIds.slice(i, i + B).map(id => `'${id}'`).join(',');
      leadPromises.push(querySalesforce<{ Id: string; ConvertedAccountId: string | null; IsConverted: boolean }>(
        `SELECT Id, ConvertedAccountId, IsConverted FROM Lead WHERE Id IN (${batch})`
      ).catch(() => []));
    }
    const leadBatches = await Promise.all(leadPromises);
    for (const batch of leadBatches) {
      for (const l of batch) {
        if (l.IsConverted && l.ConvertedAccountId) { lead2acct.set(l.Id, l.ConvertedAccountId); accts.add(l.ConvertedAccountId); }
      }
    }

    console.log(`[CB] SF: ${lead2acct.size} converted leads, ${accts.size} accounts`);

    const oppsByAcct = new Map<string, SFOppInfo[]>();
    const acctArr = Array.from(accts);
    const oppPromises: Promise<{ Id: string; Name: string; StageName: string; Amount: number | null; CloseDate: string; Case_Type__c: string | null; AccountId: string }[]>[] = [];
    for (let i = 0; i < acctArr.length; i += B) {
      const batch = acctArr.slice(i, i + B).map(id => `'${id}'`).join(',');
      oppPromises.push(querySalesforce<{ Id: string; Name: string; StageName: string; Amount: number | null; CloseDate: string; Case_Type__c: string | null; AccountId: string }>(
        `SELECT Id, Name, StageName, Amount, CloseDate, Case_Type__c, AccountId FROM Opportunity WHERE AccountId IN (${batch}) AND StageName IN ('Retained/Won','Closed Won','Consultation Booked','Awaiting Retainer')`
      ).catch(() => []));
    }
    const oppBatches = await Promise.all(oppPromises);
    for (const batch of oppBatches) {
      for (const o of batch) {
        if (!oppsByAcct.has(o.AccountId)) oppsByAcct.set(o.AccountId, []);
        oppsByAcct.get(o.AccountId)!.push({ stage: o.StageName, amount: o.Amount || 0, closeDate: o.CloseDate || "", caseType: o.Case_Type__c || "", name: o.Name || "" });
      }
    }

    const result: ScoringWithSF[] = scoringRows.map(sr => {
      const acctId = sr.sfId ? lead2acct.get(sr.sfId) : undefined;
      const opps = acctId ? (oppsByAcct.get(acctId) || []) : [];
      return { ...sr, opps };
    });

    const withOpps = result.filter(r => r.opps.length > 0).length;
    console.log(`[CB] Scoring+SF: ${result.length} total, ${withOpps} with opps`);
    
    scoringSFCache = { data: result, ts: Date.now() };
    return result;
  } catch (e) {
    console.error('[CB] Scoring+SF error:', e);
    return scoringSFCache?.data || [];
  } finally {
...
export async function computeCommBookData(startDate?: string, endDate?: string): Promise<CommBookData> {
  ...
  const [fbIg, yt] = await Promise.all([
    ...
    fetchSheet(COMMENTS_SHEET_ID, '427395799', 'K:P'),
  ]);
  
  const scoringSF = await getScoringSFData();
  const t1 = Date.now();
  ...
  for (const c of allComments) {
    ...
    // Visa extraction deferred to Salesforce join (Comment History not fetched for speed)
  }

  const BOOK = ["Consultation Booked", "Awaiting Retainer", "Retained/Won", "Closed Won"];
  const PAID = ["Awaiting Retainer", "Retained/Won", "Closed Won"];
  const RET = ["Retained/Won", "Closed Won"];

  let filteredScoring = scoringSF;
  if (doDateFilter) {
    filteredScoring = scoringSF.filter(s => {
      if (s.dateCreated) {
  ...
  interface ScoringOppResult { scoring: ScoringWithSF; bestOpp?: SFOppInfo; }
  
  const scoringResults: ScoringOppResult[] = filteredScoring.map(s => {
    if (s.opps.length === 0) return { scoring: s };
    const best = s.opps.find(o => o.stage === "Retained/Won") || s.opps.find(o => o.stage === "Closed Won")
      || s.opps.find(o => o.stage === "Consultation Booked") || s.opps.find(o => o.stage === "Awaiting Retainer") || s.opps[0];
    return { scoring: s, bestOpp: best };
  });

  const booked = scoringResults.filter(r => r.bestOpp && BOOK.includes(r.bestOpp.stage)).length;
  const paid = scoringResults.filter(r => r.bestOpp && PAID.includes(r.bestOpp.stage)).length;
  const retained = scoringResults.filter(r => r.bestOpp && RET.includes(r.bestOpp.stage)).length;
  const revenue = scoringResults.filter(r => r.bestOpp && RET.includes(r.bestOpp.stage) && r.bestOpp.amount > 0)
    .reduce((s, r) => s + (r.bestOpp?.amount || 0), 0);

  const hero = {
    commBookRate: totalComments > 0 ? (booked / totalComments) * 100 : 0,
    totalComments, totalBookings: booked, totalRetainers: retained, totalRevenue: revenue,
  };
  ...
  for (const sr of scoringResults) {
    ...
    if (sr.bestOpp && BOOK.includes(sr.bestOpp.stage)) p.b++;
    if (sr.bestOpp && RET.includes(sr.bestOpp.stage)) { p.r++; if (sr.bestOpp.amount > 0) p.rev += sr.bestOpp.amount; }
  ...
  const platforms = Array.from(allPlats).map(name => {
    const cc = commentPlatCounts.get(name) || { c: 0, d: 0 };
    const pb = platBookings.get(name) || { s: 0, b: 0, r: 0, rev: 0 };
    return {
      platform: name, color: PLAT_COLORS[name] || "#767676",
      totalComments: cc.c, dmsStarted: cc.d, leadsScored: pb.s,
      consultationsBooked: pb.b, retainersSigned: pb.r,
      commBookRate: cc.c > 0 ? (pb.r / cc.c) * 100 : 0, revenue: pb.rev,
    };
  ...
  for (const sr of scoringResults) {
    let visa = "Unknown";
    if (sr.bestOpp?.caseType) visa = sr.bestOpp.caseType.toLowerCase().includes("immigration") ? extractVisa(sr.scoring.name) : sr.bestOpp.caseType;
    if (visa === "Unknown" || visa === "Other") continue;
    const v = visaAgg.get(visa) || { comments: 0, scored: 0, booked: 0, retained: 0, revenue: 0 };
    if (sr.scoring.score > 0) v.scored++;
    if (sr.bestOpp && BOOK.includes(sr.bestOpp.stage)) v.booked++;
    if (sr.bestOpp && RET.includes(sr.bestOpp.stage)) { v.retained++; if (sr.bestOpp.amount > 0) v.revenue += sr.bestOpp.amount; }
    visaAgg.set(visa, v);
  }
  ...
  const visaTypes = Array.from(visaAgg.entries())
    ...
    .map(([n, d]) => ({
      visaType: n, comments: d.comments, leadsScored: d.scored, bookings: d.booked, retainers: d.retained,
      commBookRate: d.comments > 0 ? (d.retained / d.comments) * 100 : 0,
      avgRetainerValue: d.retained > 0 ? d.revenue / d.retained : 0, totalRevenue: d.revenue,
  ...
  for (const sr of scoringResults) {
    ...
    g.c++;
    if (sr.bestOpp && BOOK.includes(sr.bestOpp.stage)) g.b++;
    if (sr.bestOpp && RET.includes(sr.bestOpp.stage)) { g.r++; if (sr.bestOpp.amount > 0) g.rev += sr.bestOpp.amount; }
    geoMap.set(region, g);
  }
  const geographic = Array.from(geoMap.entries())
    .filter(([, d]) => d.c >= 2)
    .map(([n, d]) => ({ region: n, comments: d.c, bookings: d.b, retainers: d.r, commBookRate: d.c > 0 ? (d.r / d.c) * 100 : 0, revenue: d.rev }))
    .sort((a, b) => b.revenue - a.revenue || b.retainers - a.retainers || b.comments - a.comments);

  const revenueAttribution = scoringResults
    .filter(r => r.bestOpp && RET.includes(r.bestOpp.stage) && r.bestOpp.amount > 0)
  ...
  const result: CommBookData = {
    ...
    hero, funnel, topPosts, platforms, visaTypes, geographic, revenueAttribution,
    debug: {
      source: "live-query",
      totalComments, fetchMs: t1 - t0, totalMs: Date.now() - t0,
      scoringRows: filteredScoring.length, scoringWithOpps: scoringResults.filter(r => r.bestOpp).length,
      sfCacheAge: scoringSFCache ? Math.round((Date.now() - scoringSFCache.ts) / 1000) : -1,
      dms: dmsSent, scored: leadsScored, booked, paid, retained, revenue,
    },
  };
